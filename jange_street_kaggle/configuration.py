...
class EnvConfigParser(ConfigParser):
    ...
    def _env_var_name(section, key):
        return 'JANE__{S}__{K}'.format(S=section.upper(), K=key.upper())

    def _get_env_var_option(self, section, key):
        # must have format JANE__{SECTION}__{KEY} (note double underscore)
    ...
    def getsection(self, section):
        ...
        section_prefix = 'JANE__{S}__'.format(S=section.upper())
    ...
    def as_dict(
        ...
        :type raw: bool
        :param include_env: Should the value of configuration from JANE__
        ...
        if include_env:
            for ev in [ev for ev in os.environ if ev.startswith('JANE__')]:
                try:
                    _, section, key = ev.split('__', 2)
                    opt = self._get_env_var_option(section, key)
                except ValueError:
                    continue
                if not display_sensitive and ev != 'JANE__CORE__UNIT_TEST_MODE':
                    opt = '< hidden >'
                elif raw:
                    opt = opt.replace('%', '%%')
                if display_source:
                ...
                # then we won't be able to set any Jane Street Kaggle competition environment
                # variables. Jane Street Kaggle competition only parse environment variables starts
                # with JANE_. Therefore, we need to make it a special case.
...
def get_home_dir():
    rv = expand_env_var(os.environ.get('JANE_HOME', os.getcwd()))
